...
async function initializeSupabaseClient() {
  try {
    ...
    // Cek apakah Supabase JS sudah di-load
    if (typeof supabase === 'undefined') {
      console.error('❌ Supabase JS library belum di-load!');
      console.error('Pastikan Anda menambahkan:');
      console.error('<script src="https://unpkg.com/@supabase/supabase-js"></script>');
      console.error('sebelum app.js di index.html');
      return false;
    }
...
const SupabaseFunctions = {
  ...
  initializeDatabase: async function() {
    try {
      ...
      // Jika error 42P01 berarti tabel tidak ada
      if (error && error.code === '42P01') {
        console.warn('Tabel barang belum dibuat di database');
        return false;
      }
      
      return !error;
    } catch (error) {
      console.error('Error initializeDatabase:', error);
      return false;
    }
  },
  
  // Get barang dengan join kategori dan konversi
  getBarang: async function() {
    try {
      ...
      console.error('Exception getBarang:', error);
      return [];
    }
  },
  
  // Get barang masuk dengan filter
  getBarangMasuk: async function(filter = {}) {
    try {
      if (!supabaseClient) {
        console.error('Supabase client belum diinisialisasi');
        return [];
      ...
      if (error) {
        console.error('Error getBarangMasuk:', error);
        return [];
      }
      
      return data || [];
    } catch (error) {
      console.error('Exception getBarangMasuk:', error);
      return [];
    }
  },
  
  // Get barang keluar dengan filter
  getBarangKeluar: async function(filter = {}) {
    try {
      if (!supabaseClient) {
        console.error('Supabase client belum diinisialisasi');
        return [];
      ...
      console.error('Exception getBarangKeluar:', error);
      return [];
    }
  },
  
  // Get laporan data
  getLaporan: async function(startDate, endDate, reportType = 'all') {
    try {
      if (!supabaseClient) {
        console.error('Supabase client belum diinisialisasi');
        return [];
      ...
      // Combine barang masuk dan keluar
      ...
      // Filter by date
      const filteredData = [
        ...masukData.map(item => ({ ...item, jenis: 'MASUK', satuan: 'Dus' })),
        ...keluarData.map(item => ({ ...item, jenis: 'KELUAR', satuan: item.satuan_keluar }))
      ].filter(item => {
      ...
      if (reportType === 'in') {
        return filteredData.filter(item => item.jenis === 'MASUK');
      } else if (reportType === 'out') {
        return filteredData.filter(item => item.jenis === 'KELUAR');
      }
  ...
  getKategori: async function() {
    try {
      ...
      console.error('Exception getKategori:', error);
      return [];
    }
  },
  
  // Get satuan konversi
  getSatuanKonversi: async function() {
    try {
      if (!supabaseClient) {
        console.error('Supabase client belum diinisialisasi');
        return [];
      ...
      if (error) {
        console.error('Error getSatuanKonversi:', error);
        return [];
      }
      
      return data || [];
    } catch (error) {
      console.error('Exception getSatuanKonversi:', error);
      return [];
    }
  },
  
  // Get konversi for specific barang
  getKonversiBarang: async function(barangId) {
    try {
      if (!supabaseClient) {
        console.error('Supabase client belum diinisialisasi');
        return [];
      ...
      const { data, error } = await supabaseClient
        ...
        .order('jumlah_per_dus', { ascending: false });
      
      if (error) {
        console.error('Error getKonversiBarang:', error);
        return [];
      }
      
      return data || [];
    } catch (error) {
      console.error('Exception getKonversiBarang:', error);
      return [];
    }
  },
  
  // Add barang
  addBarang: async function(barangData) {
    try {
      if (!supabaseClient) {
        console.error('Supabase client belum diinisialisasi');
        return false;
      ...
      if (barangError) {
        console.error('Error addBarang:', barangError);
        throw barangError;
      }
      
      // Then add konversi satuan
      const { error: konversiError } = await supabaseClient
        .from('satuan_konversi')
        .insert({
          barang_id: barang.id,
          jumlah_per_dus: barangData.jumlah_per_dus || 24,
          satuan_unit: barangData.satuan_unit || 'Pcs'
        });
      
      if (konversiError) {
        console.error('Error adding konversi:', konversiError);
        // Still return true since barang was added
      }
      
      return true;
      
    } catch (error) {
      console.error('Exception addBarang:', error);
      throw error;
    }
  },
  
  // Update barang
  updateBarang: async function(id, barangData) {
    try {
      if (!supabaseClient) {
        console.error('Supabase client belum diinisialisasi');
        return false;
      ...
      console.error('Exception updateBarang:', error);
      throw error;
    }
  },
  
  // Delete barang
  deleteBarang: async function(id) {
    try {
      if (!supabaseClient) {
        console.error('Supabase client belum diinisialisasi');
        return false;
      ...
      if (konversiError) {
        console.warn('Error deleting konversi:', konversiError);
      }
      ...
      console.error('Exception deleteBarang:', error);
      throw error;
    }
  },
  
  // Add barang masuk
  addBarangMasuk: async function(barangMasukData) {
    try {
      if (!supabaseClient) {
        console.error('Supabase client belum diinisialisasi');
        return false;
      ...
      if (error) {
        console.error('Error addBarangMasuk:', error);
        throw error;
      }
      
      // Update stock
      await this.updateStokBarang(barangMasukData.barang_id, barangMasukData.total_pcs, 'in');
      
      return true;
    } catch (error) {
      console.error('Exception addBarangMasuk:', error);
      throw error;
    }
  },
  
  // Add multiple barang masuk
  addMultipleBarangMasuk: async function(items, tanggal, supplier, keterangan) {
    try {
      if (!supabaseClient) {
        console.error('Supabase client belum diinisialisasi');
        return false;
      ...
      const barangMasukItems = items.map(item => ({
      ...
      const { data, error } = await supabaseClient
        .from('barang_masuk')
        .insert(barangMasukItems)
        .select();
      
      if (error) {
        console.error('Error addMultipleBarangMasuk:', error);
        throw error;
      }
      
      // Update stock for each item
      for (const item of items) {
        await this.updateStokBarang(item.barang_id, item.total_pcs, 'in');
      }
      
      return true;
    } catch (error) {
      console.error('Exception addMultipleBarangMasuk:', error);
      throw error;
    }
  },
  
  // Add barang keluar
  addBarangKeluar: async function(barangKeluarData) {
    try {
      if (!supabaseClient) {
        console.error('Supabase client belum diinisialisasi');
        return false;
      ...
      if (error) {
        console.error('Error addBarangKeluar:', error);
        throw error;
      }
      
      // Update stock
      await this.updateStokBarang(barangKeluarData.barang_id, barangKeluarData.total_pcs, 'out');
      
      return true;
    } catch (error) {
      console.error('Exception addBarangKeluar:', error);
      throw error;
    }
  },
  
  // Add multiple barang keluar
  addMultipleBarangKeluar: async function(items, tanggal, penerima, keterangan) {
    try {
      if (!supabaseClient) {
        console.error('Supabase client belum diinisialisasi');
        return false;
      ...
      const barangKeluarItems = items.map(item => ({
        barang_id: item.barang_id,
      ...
      const { data, error } = await supabaseClient
        .from('barang_keluar')
        .insert(barangKeluarItems)
        .select();
      
      if (error) {
        console.error('Error addMultipleBarangKeluar:', error);
        throw error;
      }
      
      // Update stock for each item
      for (const item of items) {
        await this.updateStokBarang(item.barang_id, item.total_pcs, 'out');
      }
      
      return true;
    } catch (error) {
      console.error('Exception addMultipleBarangKeluar:', error);
      throw error;
    }
  },
  
  // Add kategori
  addKategori: async function(kategoriData) {
    try {
      if (!supabaseClient) {
        console.error('Supabase client belum diinisialisasi');
        return false;
      ...
      console.error('Exception addKategori:', error);
      throw error;
    }
  },
  
  // Update kategori
  updateKategori: async function(id, kategoriData) {
    try {
      if (!supabaseClient) {
        console.error('Supabase client belum diinisialisasi');
        return false;
      ...
      console.error('Exception updateKategori:', error);
      throw error;
    }
  },
  ...
  deleteKategori: async function(id) {
    try {
      ...
      const { data: barang, error: checkError } = await supabaseClient
        .from('barang')
        .select('id')
        .eq('kategori_id', id)
        .limit(1);
      ...
      if (checkError) {
        console.error('Error checking kategori usage:', checkError);
        throw checkError;
      }
      
      if (barang && barang.length > 0) {
        throw new Error('Kategori sedang digunakan oleh barang. Tidak dapat dihapus.');
      }
      ...
      console.error('Exception deleteKategori:', error);
      throw error;
    }
  },
  
  // Add satuan konversi
  addSatuanKonversi: async function(satuanData) {
    try {
      if (!supabaseClient) {
        console.error('Supabase client belum diinisialisasi');
        return false;
      ...
      // Check if konversi already exists for this barang
      const { data: existing, error: checkError } = await supabaseClient
        .from('satuan_konversi')
        .select('id')
        .eq('barang_id', satuanData.barang_id)
        .eq('satuan_unit', satuanData.satuan_unit)
        .limit(1);
      
      if (checkError) {
        console.error('Error checking existing konversi:', checkError);
        throw checkError;
      }
      
      if (existing && existing.length > 0) {
        throw new Error('Konversi untuk satuan ini sudah ada');
      }
      ...
      if (error) {
        console.error('Error addSatuanKonversi:', error);
        throw error;
      }
      
      return true;
    } catch (error) {
      console.error('Exception addSatuanKonversi:', error);
      throw error;
    }
  },
  
  // Update satuan konversi
  updateSatuanKonversi: async function(id, satuanData) {
    try {
      if (!supabaseClient) {
        console.error('Supabase client belum diinisialisasi');
        return false;
      ...
      if (error) {
        console.error('Error updateSatuanKonversi:', error);
        throw error;
      }
      
      return true;
    } catch (error) {
      console.error('Exception updateSatuanKonversi:', error);
      throw error;
    }
  },
  
  // Delete satuan konversi
  deleteSatuanKonversi: async function(id) {
    try {
      if (!supabaseClient) {
        console.error('Supabase client belum diinisialisasi');
        return false;
      ...
      if (error) {
        console.error('Error deleteSatuanKonversi:', error);
        throw error;
      }
      
      return true;
    } catch (error) {
      console.error('Exception deleteSatuanKonversi:', error);
      throw error;
    }
  },
  
  // Update stock barang
  updateStokBarang: async function(barangId, jumlah, type) {
    try {
      if (!supabaseClient) {
        console.error('Supabase client belum diinisialisasi');
        return false;
      ...
      // Get current stock
      const { data: barang, error: getError } = await supabaseClient
        .from('barang')
        .select('stok_pcs')
        .eq('id', barangId)
        .single();
      ...
      // Calculate new stock
      const currentStock = barang.stok_pcs || 0;
      let newStock;
      ...
      console.error('Exception updateStokBarang:', error);
      throw error;
    }
  }
};
...
document.addEventListener('DOMContentLoaded', async function() {
  ...
  try {
    // Inisialisasi Supabase Client terlebih dahulu
    const supabaseInitialized = await initializeSupabaseClient();
    
    if (!supabaseInitialized) {
      console.warn('⚠️ Supabase tidak terinisialisasi, menggunakan mode offline');
    }
...
async function initializeApp() {
  ...
  try {
    ...
    const isInitialized = await supabaseFunctions.initializeDatabase();
    
    if (!isInitialized) {
      console.warn('⚠️ Database belum diinisialisasi atau tabel belum dibuat.');
      showNotification('Database belum sepenuhnya diinisialisasi. Pastikan tabel sudah dibuat dengan SQL yang disediakan.', 'warning');
    } else {
      console.log('✅ Aplikasi Inventori siap digunakan!');
...
async function loadDashboardData() {
  ...
  try {
    ...
    try {
      [barang, barangMasuk, barangKeluar] = await Promise.all([
        supabaseFunctions.getBarang(),
        supabaseFunctions.getBarangMasuk(),
        supabaseFunctions.getBarangKeluar()
      ]);
    } catch (error) {
      console.error('Error loading dashboard data:', error);
      showNotification('Gagal memuat sebagian data dashboard', 'warning');
    }
    ...
    // Tampilkan pesan error di aktivitas
    ...
    if (activityList) {
      activityList.innerHTML = `
        <div class="loading error">
          ...
          <p>Silakan cek koneksi database</p>
        </div>
      `;
    }
  }
...
function updateStatsLoading() {
  const stats = ['total-barang', 'total-masuk', 'total-keluar', 'total-rendah'];
  stats.forEach(id => {
    const element = document.getElementById(id);
    if (element) element.textContent = '...';
  });
  
  // Tampilkan loading di aktivitas
  const activityList = document.getElementById('activity-list');
  if (activityList) {
    activityList.innerHTML = `
      <div class="loading">
...
async function loadBarangData() {
  ...
  try {
    const barang = await supabaseFunctions.getBarang();
    ...
    if (tbody) {
      tbody.innerHTML = '<tr><td colspan="8" class="loading error">Error memuat data barang</td></tr>';
    }
  }
}
...
async function loadBarangMasukData() {
  ...
  try {
    ...
    const barangMasuk = await supabaseFunctions.getBarangMasuk(filter);
    ...
    if (tbody) {
      tbody.innerHTML = '<tr><td colspan="8" class="loading error">Error memuat data barang masuk</td></tr>';
    }
  }
}
...
async function loadBarangKeluarData() {
  ...
  try {
    ...
    const barangKeluar = await supabaseFunctions.getBarangKeluar(filter);
    ...
    if (tbody) {
      tbody.innerHTML = '<tr><td colspan="8" class="loading error">Error memuat data barang keluar</td></tr>';
    }
  }
}
...
async function loadLaporanData() {
  ...
  try {
    ...
    const laporan = await supabaseFunctions.getLaporan(startDate, endDate, reportType);
    ...
    if (tbody) {
      tbody.innerHTML = '<tr><td colspan="7" class="loading error">Error memuat data laporan</td></tr>';
    }
  }
}
...
async function loadKategoriData() {
  console.log('🏷️ Memuat data kategori...');
  
  try {
    const kategori = await supabaseFunctions.getKategori();
    ...
    if (kategoriList) {
      kategoriList.innerHTML = '<li class="loading error">Error memuat data kategori</li>';
    }
  }
}
...
async function loadSatuanData() {
  console.log('⚖️ Memuat data satuan...');
  
  try {
    const satuan = await supabaseFunctions.getSatuanKonversi();
    const barang = await supabaseFunctions.getBarang();
    ...
    if (tbody) {
      tbody.innerHTML = '<tr><td colspan="3" class="loading error">Error memuat data satuan</td></tr>';
    }
  }
}
...
async function loadAktivitasTerbaru() {
  ...
  try {
    ...
    const barangMasuk = await supabaseFunctions.getBarangMasuk({});
    const barangKeluar = await supabaseFunctions.getBarangKeluar({});
...
async function updateKonversiMasuk() {
  ...
  try {
    // Get konversi data
    const konversiData = await supabaseFunctions.getKonversiBarang(barangId);
...
async function updateInfoKeluar() {
  ...
  try {
    // Get barang data dengan fallback
    let barang = null;
    
    // Method 1: Get from Supabase if available
    if (supabaseClient) {
      const { data, error } = await supabaseClient
        .from('barang')
        .select('stok_pcs')
      ...
      if (!error) barang = data;
    }
    
    // Method 2: Get from functions if Supabase failed
    if (!barang) {
      const allBarang = await supabaseFunctions.getBarang();
      barang = allBarang.find(b => b.id == barangId) || {};
    }
    ...
    if (konversiData && konversiData.length > 0) {
      ...
      const stokDus = (barang.stok_pcs || 0) / (konversi.jumlah_per_dus || 1);
      ...
      if (stokInfoPcs) stokInfoPcs.textContent = barang.stok_pcs || 0;
  ...
  try {
    ...
    if (konversiData && konversiData.length > 0) {
      ...
      if (selectedKonversi) {
        ...
        if (totalPcsCell) totalPcsCell.textContent = Math.round(totalPcs);
        if (stokCell) {
          const stokDus = (barang.stok_pcs || 0) / selectedKonversi.jumlah_per_dus;
          stokCell.textContent = `${barang.stok_pcs || 0} Pcs (${stokDus.toFixed(2)} Dus)`;
          
          // Highlight if insufficient stock
          if (totalPcs > (barang.stok_pcs || 0)) {
...
async function handleTambahBarang(e) {
  ...
  try {
    ...
    if (isEditMode) {
      ...
      console.log('✏️ Updating barang dengan ID:', isEditMode);
      
      // Remove konversi fields from barang object
      const { jumlah_per_dus, satuan_unit, ...barangData } = barang;
      
      const result = await supabaseFunctions.updateBarang(isEditMode, barangData);
      ...
      const result = await supabaseFunctions.addBarang(barang);
...
async function handleTambahMasuk(e) {
  ...
  try {
    ...
    // Get konversi data
    const konversiData = await supabaseFunctions.getKonversiBarang(barangMasuk.barang_id);
    ...
    const result = await supabaseFunctions.addBarangMasuk(barangMasuk);
...
async function handleMultiMasuk() {
  ...
  try {
    ...
    const result = await supabaseFunctions.addMultipleBarangMasuk(items, tanggal, supplier, keteranganUmum);
...
async function handleTambahKeluar(e) {
  ...
  try {
    ...
    // Get konversi data
    const konversiData = await supabaseFunctions.getKonversiBarang(barangKeluar.barang_id);
    ...
    // Check stock availability dengan fallback
    let currentStock = 0;
    try {
      if (supabaseClient) {
        ...
        if (!error) currentStock = barang.stok_pcs || 0;
      } else {
        const allBarang = await supabaseFunctions.getBarang();
        const barang = allBarang.find(b => b.id == barangKeluar.barang_id);
        currentStock = barang?.stok_pcs || 0;
      }
    } catch (error) {
      console.error('Error checking stock:', error);
    }
    
    if (currentStock < barangKeluar.total_pcs) {
      showNotification(`Stok tidak mencukupi. Stok tersedia: ${currentStock} Pcs`, 'warning');
      return;
    }
    
    console.log('➕ Adding barang keluar:', barangKeluar);
    const result = await supabaseFunctions.addBarangKeluar(barangKeluar);
  ...
  try {
    ...
    for (const row of rows) {
      ...
      if (currentStock < totalPcs) {
        showNotification(`Stok tidak mencukupi untuk barang ${barangSelect.selectedOptions[0]?.text}. Stok tersedia: ${currentStock} Pcs`, 'warning');
        valid = false;
        break;
      }
    ...
    const result = await supabaseFunctions.addMultipleBarangKeluar(items, tanggal, penerima, keteranganUmum);
...
async function handleKategoriForm(e) {
  ...
  try {
    ...
    if (kategoriId) {
      ...
      console.log('✏️ Updating kategori dengan ID:', kategoriId);
      result = await supabaseFunctions.updateKategori(kategoriId, kategori);
      if (result) {
      ...
      result = await supabaseFunctions.addKategori(kategori);
      if (result) {
...
async function handleSatuanForm(e) {
  ...
  try {
    ...
    if (satuanId) {
      ...
      console.log('✏️ Updating satuan dengan ID:', satuanId);
      result = await supabaseFunctions.updateSatuanKonversi(satuanId, satuan);
      if (result) {
      ...
      result = await supabaseFunctions.addSatuanKonversi(satuan);
      if (result) {
...
async function editBarang(id) {
  try {
    console.log('✏️ Loading barang untuk edit ID:', id);
    ...
    if (!barang) {
      const allBarang = await supabaseFunctions.getBarang();
      barang = allBarang.find(b => b.id == id);
    }
    ...
    // Get konversi data
    const konversiData = await supabaseFunctions.getKonversiBarang(id);
    if (konversiData && konversiData.length > 0) {
      const konversi = konversiData[0];
...
async function editKategori(id) {
  try {
    console.log('✏️ Loading kategori untuk edit ID:', id);
    
    // Get kategori data dengan fallback
    let kategori = null;
    ...
    if (supabaseClient) {
      ...
      if (!error) kategori = data;
    }
    
    if (!kategori) {
      const allKategori = await supabaseFunctions.getKategori();
      kategori = allKategori.find(k => k.id == id);
    }
...
async function editSatuan(id) {
  try {
    console.log('✏️ Loading satuan untuk edit ID:', id);
    
    // Get satuan data dengan fallback
    let satuan = null;
    ...
    if (supabaseClient) {
      ...
      if (!error) satuan = data;
    }
    
    if (!satuan) {
      const allSatuan = await supabaseFunctions.getSatuanKonversi();
      satuan = allSatuan.find(s => s.id == id);
    }
    
    if (!satuan) {
      showNotification('Konversi satuan tidak ditemukan', 'error');
      return;
    }
...
async function deleteBarang(id) {
  try {
    ...
    const success = await supabaseFunctions.deleteBarang(id);
...
async function deleteBarangMasuk(id) {
  try {
    console.log('🗑️ Deleting barang masuk ID:', id);
    ...
    if (supabaseClient) {
      ...
      if (!error) transaksi = data;
    }
    
    if (!transaksi) {
      const allMasuk = await supabaseFunctions.getBarangMasuk({});
      transaksi = allMasuk.find(m => m.id == id);
    }
    
    if (!transaksi) {
      showNotification('Data barang masuk tidak ditemukan', 'error');
      return;
    }
    ...
    if (supabaseClient) {
      ...
      // Fallback: use functions
      showNotification('Fitur hapus tidak tersedia dalam mode offline', 'error');
      return;
    }
    
    // Update stock (deduct)
    await supabaseFunctions.updateStokBarang(transaksi.barang_id, transaksi.total_pcs, 'out');
...
async function deleteBarangKeluar(id) {
  try {
    console.log('🗑️ Deleting barang keluar ID:', id);
    ...
    if (!transaksi) {
      const allKeluar = await supabaseFunctions.getBarangKeluar({});
      transaksi = allKeluar.find(k => k.id == id);
    }
    
    if (!transaksi) {
      showNotification('Data barang keluar tidak ditemukan', 'error');
      return;
    }
    ...
    await supabaseFunctions.updateStokBarang(transaksi.barang_id, transaksi.total_pcs, 'in');
...
async function deleteKategori(id) {
  try {
    console.log('🗑️ Deleting kategori ID:', id);
    
    await supabaseFunctions.deleteKategori(id);
...
async function deleteSatuan(id) {
  try {
    console.log('🗑️ Deleting satuan ID:', id);
    
    await supabaseFunctions.deleteSatuanKonversi(id);
...
function debugApp() {
  ...
  console.log('Supabase Client:', supabaseClient ? '✅ Tersedia' : '❌ Tidak tersedia');
  console.log('Supabase Functions:', supabaseFunctions ? '✅ Tersedia' : '❌ Tidak tersedia');
  console.log('Window.supabaseFunctions:', window.supabaseFunctions ? '✅ Tersedia' : '❌ Tidak tersedia');
  console.log('Supabase Config:', SUPABASE_CONFIG);
  console.groupEnd();
}

// Panggil debug saat halaman selesai dimuat
